...
const WORKOUTS: Record<number, any> = {
  1: {
    name: 'LEGS',
    label: 'MONDAY · LEGS DAY',
    subtitle: '75 MIN · QUADS / HAMS / GLUTES / CALVES',
    exercises: [
  ...
  4: {
    name: 'PUSH',
    label: 'THURSDAY · PUSH DAY',
    subtitle: '75 MIN · CHEST / SHOULDERS / TRICEPS',
    exercises: [
  ...
  5: {
    ...
    subtitle: '75 MIN · BACK / BICEPS / FOREARMS',
    exercises: [
...
// Auto-adapt: given minutes remaining, compress the workout
function adaptWorkout(workout: any, minutesRemaining: number) {
  if (minutesRemaining >= 60) return workout; // full
  const exercises = workout.exercises.map((ex: any) => ({ ...ex }));
  if (minutesRemaining >= 45) {
    // cut 1 set from each accessory
    return { ...workout, exercises: exercises.map((ex: any) => ex.isMain ? ex : { ...ex, sets: Math.max(2, ex.sets - 1) }) };
  }
  if (minutesRemaining >= 30) {
    // cut last accessory, reduce all accessory to 2 sets
  ...
  if (minutesRemaining >= 15) {
    // main lifts only, 3 sets
    return { ...workout, exercises: exercises.filter((e: any) => e.isMain).map((ex: any) => ({ ...ex, sets: 3 })) };
  }
  // <15 min: skip
  return null;
}
...
function getTodayWorkout() {
  ...
  return [1, 4, 5].includes(day) ? getWorkoutForDay(day) : getWorkoutForDay(5);
}
...
export default function GymScreen() {
  ...
  async function loadData() {
    ...
    if (data) {
      setHistory(data);
      // rebuild PRs from history
      const prMap: Record<string, { w: number; r: number }> = {};
      data.forEach((session: any) => {
        if (session.set_log) {
          Object.entries(session.set_log).forEach(([exName, rows]: any) => {
            rows.forEach((row: any) => {
              if (row.done && row.w && row.r) {
                const w = parseFloat(row.w), r = parseInt(row.r);
                if (!prMap[exName] || w > prMap[exName].w || (w === prMap[exName].w && r > prMap[exName].r)) {
                  prMap[exName] = { w, r };
                }
              }
            });
  ...
  function startSession() {
    ...
    const now = new Date();
    const minutesLeft = 75;
    const adapted = adaptWorkout(w, minutesLeft);
    if (!adapted) {
      Alert.alert('Not enough time', 'Less than 15 minutes — session skipped. No guilt.');
      return;
    }
    setSession(adapted);
    setSessionStart(now);
  ...
  function checkPR(name: string, w: number, r: number) {
    setPrs(prev => {
      const pr = prev[name];
      if (!pr || w > pr.w || (w === pr.w && r > pr.r)) {
        return { ...prev, [name]: { w, r } };
      }
      return prev;
    });
  }
  ...
  async function finishSession() {
    ...
    await supabase.from('gym_sessions').insert(entry);
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={s.todayCard}>
            <View style={s.tcHeader}>
              <View>
                <Text style={s.tcLabel}>TODAY'S WORKOUT</Text>
                <Text style={s.tcTitle}>{today.name} DAY</Text>
              </View>
          ...
          <TouchableOpacity style={s.lfgBtn} onPress={() => navigation.navigate('LFG')}>
            <Text style={s.lfgBtnText}>🔥 LFG</Text>
          <TouchableOpacity style={s.histBtn} onPress={() => setScreen('beast')}>
      ...
      <SafeAreaView style={s.bg}>
        ...
        <View style={s.wkHeader}>
          ...
          <View>
            ...
            <Text style={s.wkSub}>{new Date().toLocaleDateString('en-CA', { weekday: 'short', month: 'short', day: 'numeric' }).toUpperCase()}</Text>
          </View>
        </View>
        ...
        <View style={s.timerBar}>
          ...
          <View style={s.restBtns}>
            <TouchableOpacity style={s.rBtn} onPress={() => startRest(restTimes.accessory)}>
              <Text style={s.rBtnText}>{restTimes.accessory}s</Text>
            </TouchableOpacity>
            <TouchableOpacity style={s.rBtn} onPress={() => startRest(restTimes.main === 120 ? 120 : 120)}>
              <Text style={s.rBtnText}>2m</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[s.rBtn, s.rBtnGold]} onPress={() => startRest(restTimes.main)}>
              <Text style={[s.rBtnText, { color: C.black }]}>{Math.floor(restTimes.main / 60)}m</Text>
            </TouchableOpacity>
          </View>
        </View>
            ...
            return (
              <View key={ei} style={[s.exCard, isActive && s.exCardActive]}>
                <TouchableOpacity style={s.exCardHead} onPress={() => setActiveEx(ei)}>
                  <View style={{ flex: 1 }}>
                    <Text style={s.ecTitle}>{ex.name}{ex.isWeak ? ' ⚡' : ''}{ex.isMain ? ' 💪' : ''}</Text>
                    ...
                    {rows.map((row: any, si: number) => (
                      <View key={si} style={s.setRow}>
                        ...
                        <TextInput
                          ...
                          value={row.w}
                          onChangeText={v => updateSetField(ei, si, 'w', v)}
                        />
                        <TextInput
                          style={s.setIn}
                          placeholder="reps"
                          ...
                          value={row.r}
                          onChangeText={v => updateSetField(ei, si, 'r', v)}
                        />
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        {history.length === 0 ? (
          ...
          <View key={i} style={s.histCard}>
            ...
            {h.prs_hit?.length > 0 && (
              <View style={s.prRow}>
                {h.prs_hit.map((p: string, j: number) => (
                  <View key={j} style={s.prTag}><Text style={s.prTagText}>🏆 {p}</Text></View>
                ))}
              </View>
            )}
          </View>
